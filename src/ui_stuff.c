...
void slider_widget(UiRect r, SliderState *slider_state) {
  ...
  if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
    Vector2 mouse_position_down = GetMousePosition();
    if (Vector2Distance(mouse_position_down, knob_position) <= knob_radius) {
...
void oct_trans_button_widget(UiRect r, int *octave, bool *left_is_pressed, bool *right_is_pressed)
{
    ...
    Vector2 mouse_position_down = {-1.0, -1.0};

    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        mouse_position_down = GetMousePosition();

        if (CheckCollisionPointRec(mouse_position_down, right_button)) {
            *right_is_pressed = true;
            *octave = (*octave >= 6) ? 6 : (*octave + 1);;
        }

        if (CheckCollisionPointRec(mouse_position_down, left_button)) {
...
void keyboard_widget(UiRect rect,
                     KeyboardPressedKeyMap** keys_map_out,
                     int* last_mouse_down_key,
...
*/
   ...
   for(size_t i = 0; i < hmlen(keys_map_in); i++) {
       if(octave_offset <= keys_map_in[i].key && keys_map_in[i].key < octave_offset + 12) {
           ...
           hmput(collision_keys_map_in, collision_key_in, KEY_PRESSED);
       }
   }
   ...
   size_t key_index = 0;
   size_t collision_key_down = -1;
   size_t collision_key_up = -1;

   Vector2 mouse_position_down = {-1.0, -1.0};
   Vector2 mouse_position_up = {-1.0, -1.0};

   if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
       mouse_position_down = GetMousePosition();
   }
   if (IsMouseButtonReleased(MOUSE_BUTTON_LEFT)) {
       mouse_position_up = GetMousePosition();
   }

   // "white" keys
   for(size_t i = 0; i < 7; ++i) {
       white_key.x =  i*white_key_w + x;
       if (CheckCollisionPointRec(mouse_position_down, white_key)) {
           collision_key_down = key_index;
       }
       if (CheckCollisionPointRec(mouse_position_up, white_key)) {
           collision_key_up = key_index;
       }
       ++key_index;
   }
   ...
   for(size_t i = 0; i < 6; ++i){
       if(i != 2){
           ...
           if (CheckCollisionPointRec(mouse_position_down, black_key)) {
               collision_key_down = key_index;
           }
           if (CheckCollisionPointRec(mouse_position_up, black_key)) {
               collision_key_up = key_index;
           }
           ++key_index;
       }
   ...
   if (collision_key_down == -1) {
       //TODO check if necessary

   } else {
       size_t key = key_lookup[collision_key_down] + octave*12;
       if(!hmgetp_null(*keys_map_out, key)) {
           hmput(*keys_map_out, key, KEY_PRESSED);
           if ((*last_mouse_down_key != -1) && (*last_mouse_down_key != key)) {
               // the old last_mouse_down_key needs to be released
               if(hmgetp_null(*keys_map_out, *last_mouse_down_key)) {
                   hmput(*keys_map_out, *last_mouse_down_key, KEY_RELEASED);
               }
           }
   ...
   if (collision_key_up != -1) {
       size_t key = key_lookup[collision_key_up] + octave*12;
       if(hmgetp_null(*keys_map_out, key)) {
           hmput(*keys_map_out, key, KEY_RELEASED);
       }
       *last_mouse_down_key = -1;
   }
   // TODO check if we slide to another key
...
{
    LayoutStack ls = {0};
    layout_stack_push(&ls, LO_VERT, rect, 4, 0);
    slider_widget(layout_stack_slot(&ls), base_amp);
    slider_widget(layout_stack_slot(&ls), ov1_amp);
    slider_widget(layout_stack_slot(&ls), ov2_amp);
    slider_widget(layout_stack_slot(&ls), noise_amp);
    layout_stack_pop(&ls);
    layout_stack_delete(&ls);
}
...
void decay_widget(UiRect rect, SliderState* base_decay, SliderState* ov1_decay, SliderState* ov2_decay, SliderState* noise_decay)
{
    LayoutStack ls = {0};
    layout_stack_push(&ls, LO_VERT, rect, 4, 0);
    slider_widget(layout_stack_slot(&ls), base_decay);
    slider_widget(layout_stack_slot(&ls), ov1_decay);
    slider_widget(layout_stack_slot(&ls), ov2_decay);
    slider_widget(layout_stack_slot(&ls), noise_decay);
    layout_stack_pop(&ls);
    layout_stack_delete(&ls);
}
