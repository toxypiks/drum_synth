...
int main(void) {
    ...
    KeyboardPressedKeyMap* keys_map_out = NULL;
    int last_mouse_down_key = -1;
    ...
    while(!WindowShouldClose()) {
        ...
        // send drum_msgs
        if (play_is_pressed == true && play_is_pressed_last == false) {
            drum_msg.base_amp = base_amp.scroll;
            drum_msg.ov1_amp = ov1_amp.scroll;
            drum_msg.ov2_amp = ov2_amp.scroll;
            drum_msg.noise_amp = noise_amp.scroll;
            drum_msg.ov1_decay = ov1_decay.scroll;
            drum_msg.ov2_decay = ov2_decay.scroll;
        ...
        for (size_t i = 0; i < hmlen(keys_map_out); i++) {
            if(keys_map_out[i].value == KEY_PRESSED) {
                // send NoteOn msg for "fresh" keys
                keys_map_out[i].value = KEY_STILL_PRESSED;
                MidiMsg midi_msg_out = {
                    .key  = keys_map_out[i].key,
                    .vel   = 1.0f,
                    .is_on = true,
                    .time_stamp = 0
                };
                int ret_midi_msg = lf_queue_push(&thread_stuff->model_msg_queue, "midi_msg", (void*)&midi_msg_out, sizeof(MidiMsg));
            } else if (keys_map_out[i].value == KEY_RELEASED) {
                // send NoteOff msg for released keys
                MidiMsg midi_msg_out = {
                    .key  = keys_map_out[i].key,
                    .vel   = 1.0f,
                    .is_on = false,
                ...
                int ret_midi_msg = lf_queue_push(&thread_stuff->model_msg_queue, "midi_msg", (void*)&midi_msg_out, sizeof(MidiMsg));
                printf("keys_map_out len: %d delete key: %d!!!!!!!!!!!!!!!!!!!!!!\n", hmlen(keys_map_out), keys_map_out[i].key);
                hmdel(keys_map_out, keys_map_out[i].key);
                printf("keys_map_out len: %d ?!?!?!?!?!?!?!!!!!!!!!!!!!!!!!!!!!!!\n", hmlen(keys_map_out));
            }
        }
        ...
        amplitude_widget(layout_stack_slot(&ls),
                     &base_amp,
                     &ov1_amp,
                     &ov2_amp,
                     &noise_amp);

        decay_widget(layout_stack_slot(&ls),
                     &base_decay,
                     &ov1_decay,
                     &ov2_decay,
                     &noise_decay);
